...
class RGBTMaskFormer(nn.Module):
    ...
    def from_config(cls, cfg):
        backbone = build_backbone(cfg)  # "RGBTSwinTransformer"
        sem_seg_head = build_sem_seg_head(cfg, backbone.output_shape())   # "MaskFormerHead" 
    ...
    def forward(self, batched_inputs):
        ...
        images = ImageList.from_tensors(images, self.size_divisibility)  # zero-padding to let the size % size_divisibility = 0 (32 here)

        if self.training:
            features, attention_maps  = self.backbone(images.tensor)
            ...
            return losses_rgbt , attention_maps
        else:
            features, attention_maps  = self.backbone(images.tensor)
            outputs = self.sem_seg_head(features)
            ...
            return processed_results , attention_maps 
