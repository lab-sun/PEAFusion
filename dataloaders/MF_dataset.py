...
class MF_dataset(Dataset):
    ...
    def __init__(self, data_dir, cfg, split):
        ...
        if cfg.INPUT.COLOR_AUG_SSD:    # default: True   color augmentation
    ...
    def __getitem__(self, index):
        ...
        image      = torch.as_tensor(np.ascontiguousarray(image.transpose(2, 0, 1)))  # (H,W,C) -> (C,H,W)
        sem_seg_gt = torch.as_tensor(sem_seg_gt.astype("long"))

        if self.size_divisibility > 0:
            image_size = (image.shape[-2], image.shape[-1])  # (H,W)
            padding_size = [
                0,
        ...
        if sem_seg_gt is not None:
            ...
            classes = np.unique(sem_seg_gt)   # obtain all the classes based on unique pixel value on label
        ...
        return result   #  {"name":, "image":, "sem_seg_gt":, "instances": (gt_classes, gt_masks) }
